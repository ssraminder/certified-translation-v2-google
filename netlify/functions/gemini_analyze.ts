...
// Use an AI Studio model that exists on v1/v1beta:
const GEMINI_MODEL = process.env.GEMINI_MODEL || "gemini-1.5-flash";
// other valid choices: "gemini-1.5-pro", "gemini-1.0-pro"
...
export const handler: Handler = async (event) => {
  try {
    ...
    // 2) Call Gemini (AI Studio / Generative Language API)
...
-----`;
    ...
    try { parsed = JSON.parse(extractJson(raw)); }
      ...
      return { statusCode: 502, body: `Bad JSON from model: ${raw}` };
    }

    // 3) Persist to quote_files
    ...
    const update = {
      ...
      gem_status: "success",
      gem_message: `Gemini classification complete (${GEMINI_MODEL})`,
    ...
    // Surface the exact Gemini error in the response for easy debugging
