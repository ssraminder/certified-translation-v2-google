...
const LandingPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const nameValue = String(fd.get('name') ?? customerName ?? '');
    const emailValue = String(fd.get('email') ?? customerEmail ?? '');
    const phoneValue = String(fd.get('phone') ?? customerPhone ?? '');
    const intendedUseValue = String(fd.get('intended_use') ?? intendedUse ?? '');
    const sourceLanguageValue = String(fd.get('source_language') ?? sourceLanguage ?? '');
    const targetLanguageValue = String(fd.get('target_language') ?? targetLanguage ?? '');

    let resolvedQuoteId: string | null = null;
    try {
      resolvedQuoteId = resolveQuoteId();
    } catch {
    ...
    try{
      ...
      const saveJson = await saveQuote({
        quote_id: resolvedQuoteId || quoteId || null,
      });

      const assignedQuoteId = saveJson?.quote_id || resolvedQuoteId || quoteId;
      if (!assignedQuoteId) {
      ...
      <main className="flex-1 p-4" id="instant-quote">
        ...
        {screen === 'review' && (
          <div className="space-y-6">
            ...
            {quoteId && (
              <div id="quote-id" aria-label="Quote ID" style={{ marginBottom: 8 }}>
