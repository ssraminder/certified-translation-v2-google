...
// Debug helper for fetch calls – easy to remove later
...
async function loadDropdownData() {
  try {
    ...
    const { data: certRows, error: certError } = await supabase
      ...
      .select('certtype');
    const { data: useRows, error: useError } = await supabase
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log('VITE_SUPABASE_URL?', Boolean(url), 'VITE_SUPABASE_ANON_KEY?', Boolean(key));
